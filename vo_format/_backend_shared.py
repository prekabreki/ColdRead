"""Shared helpers, constants, and prompts for both analysis backends.

This module lives at the centre of the backend architecture: both
`preflight.py` (direct Anthropic API) and `claude_code_backend.py`
(local Claude Code CLI subprocess) import from it.  It contains
everything the two transports share, so the transport modules are
free to differ only in *how* they send messages and receive responses.
"""
...
DEFAULT_API_MODEL = "claude-sonnet-4-5-20250929"

API_TIMEOUT = 1200  # seconds — matches the subprocess backend default
...
# ---------------------------------------------------------------------------
# JSON extraction and validation (shared by both backends)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Message builders (shared by both backends)
# ---------------------------------------------------------------------------


def _build_preflight_message(script_text: str, filename: str) -> str:
    """Build the user message for preflight analysis.

    Handles sampling of very long scripts so both backends share identical
...
def _build_diagnostic_message(
    ...
    """Build the user message for diagnostic review.

    Both backends build the same classification list and preflight summary
...
def _build_pronunciation_message(words: list[str], script_context: str) -> str:
    """Build the user message for pronunciation guide generation.

    Deduplicates words while preserving order (same as both backends).
    """
    seen: set[str] = set()
